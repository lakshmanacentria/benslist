...
public class ImagePicker {
    ...
    public static Intent getPickImageIntent(Context context) {
        ...
        Uri uri = FileProvider.getUriForFile(context, "com.acentria.benslist.fileprovider", getTempFile(context));
        takePhotoIntent.putExtra(MediaStore.EXTRA_OUTPUT, uri/*Uri.fromFile(getTempFile(context)*/);

        intentList = addIntentsToList(context, intentList, pickIntent);
        intentList = addIntentsToList(context, intentList, takePhotoIntent);

        if (intentList.size() > 0) {
            chooserIntent = Intent.createChooser(intentList.remove(intentList.size() - 1), context.getString(R.string.pick_image_intent_chooser_title));
    ...
    public static Bitmap getImageFromResult(Context context, int resultCode, Intent imageReturnedIntent) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            ...
            if (isCamera) {     /** CAMERA **/
                selectedImage = FileProvider.getUriForFile(context, "com.acentria.benslist.fileprovider", imageFile);
                Log.v(TAG, "capture imge=> " + selectedImage);
